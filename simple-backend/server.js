...
app.get('/api/profiles', async (req, res) => {
  try {
    ...
    console.log('🔍 Total profiles in collection:', await profiles.countDocuments({}));
    
    const profilesList = await profiles
      .find(query, { 
        projection: { 
          userId: 1,  // Include userId in response
          name: 1, 
          email: 1, 
          profileImage: 1, 
          bio: 1, 
      ...
      .sort({ name: 1 })  // Sort by name instead of updatedAt
      .toArray();
    
    console.log('🔍 Found profiles:', profilesList.length);
    profilesList.forEach((profile, index) => 
      console.log(`  ${index + 1}. ${profile.name} (${profile.userId})`)
    );
